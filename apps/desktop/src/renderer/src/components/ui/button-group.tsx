...
function ButtonGroupText({ className, render, ...props }: useRender.ComponentProps<"div">) {
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex items-center gap-2 rounded-md border border-transparent bg-muted px-2.5 text-xs/relaxed font-medium [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
          className,
        ),
      },
      props,
