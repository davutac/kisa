...
const hasBroadRecordParameters = (
  parameters: readonly ESTree.TSType[]
): boolean => {
...
const isBroadIndexMember = (
  member: ESTree.TSSignature | undefined
): boolean => {
...
const isBroadRecordType = (type: ESTree.TSType): boolean => {
  ...
  if (unwrapped.type === "TSTypeReference") {
    ...
    return hasBroadRecordParameters(unwrapped.typeArguments?.params ?? []);
  }
  if (unwrapped.type !== "TSTypeLiteral" || unwrapped.members.length !== 1) {
    return false;
  }
  const [member] = unwrapped.members;
  return isBroadIndexMember(member);
};
...
): ESTree.Expression => unwrapExpressionParentheses(node.expression);
...
const directKnownValueEvidence = (
  expression: ESTree.Expression
  ...
  return unwrapped.type === "Identifier" ? undefined : null;
};
const knownValueEvidence = (
  ...
  const unwrapped = unwrapExpressionParentheses(expression);
  const directEvidence = directKnownValueEvidence(unwrapped);
...
const widenedBinding = (
  ...
  return evidence === null
    ? null
    : { boundary, broadKind, declaredAt: declarator.end, evidence };
};
