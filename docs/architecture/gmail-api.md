...
- Label definitions have account-scoped create and delete bridge methods, but no management UI yet. Creating caches Gmail's returned user label. Deleting rejects system or unknown IDs, deletes remotely, removes the definition and cached membership in one transaction, then reloads that account's mailbox list.
